...
import visitor.app.com.visitormanagement.models.City;
...
public class LoginActivity extends BaseActivity implements LoaderCallbacks<Cursor> {
    ...
    // todo mobile kotak number 9102258216
    private boolean isEmailValid(String email) {
        //TODO: Replace this with your own logic
        return UIUtil.isValidEmail(email);
    }
    ...
    private void login(String email, String password){
        ...
        Call<ApiResponse<LoginApiResponse>> call = apiService.login(email, password);
        call.enqueue(new NetworkCallback<ApiResponse<LoginApiResponse>>(this) {
            @Override
            public void onSuccess(ApiResponse<LoginApiResponse> loginApiResponseApiResponse) {
                if (loginApiResponseApiResponse.status == 0) {
                    HashMap<String, String> map = new HashMap<>();
                    map.put(Constants.AUTH_TOKEN, loginApiResponseApiResponse.apiResponseContent.bbToken);
                    UIUtil.saveDataToPreference(LoginActivity.this, map);
                    goToHome();
                    finish();
                } else {
                    handler.sendEmptyMessage(loginApiResponseApiResponse.status,
                            loginApiResponseApiResponse.message);
                }
            }
